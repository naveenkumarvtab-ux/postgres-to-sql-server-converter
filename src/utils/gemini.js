...
5. Identifier Wrapping (Rule 6): Wrap EVERY schema, table, view, function, procedure, trigger, and column identifier in square brackets consistently — e.g. [schema].[name] or [table].[column], never schema.name. Apply this even when not strictly required.
...
7. Batch Ending (Rule 11): End every CREATE VIEW / CREATE FUNCTION / CREATE PROCEDURE / CREATE TRIGGER object with GO on its own line immediately after the closing END or semicolon, since these must be the only statement in their batch in SQL Server. This is mandatory.
8. All PostgreSQL-specific functions, control flows, and syntax are fully rewritten in T-SQL (e.g. use ISNULL/COALESCE, BEGIN...END, DECLARE for variables, SET, convert PL/pgSQL loops, cursors, string concatenations).
9. Preserve the original logic, behavior, names, and types.
10. If there are features that cannot be cleanly translated to T-SQL (e.g. Postgres arrays, enums, specific regex functions, external extensions), write a visible T-SQL comment block (-- WARNING: [Explanation]) inside the code right where the issue occurs to alert the user.
11. Return ONLY the valid T-SQL script. DO NOT wrap the code in markdown code blocks (such as \`\`\`sql ... \`\`\`). Do not include any introductory or concluding text. Your entire response must be direct, executable T-SQL code.`;
